...
class Layer:
    ...
    def backward(self, target, BP=True):
        ...
        if BP:
            ...
            batch_grad = torch.einsum('ni, nj->ij', term, torch.sign(self.noise))
...
if __name__ == "__main__":
    ...
    net_act = [Activation.threshold for i in range(len(net_arc) - 1)]
    ...
    with open('LRTZ.pkl'.format(n_layers, repeat_n, method), 'wb') as file:
        pkl.dump(
            [train_loss, test_loss, acc], file)
    net.saveModel('model/LRTZ.pkl'.format(n_layers, repeat_n, method))
